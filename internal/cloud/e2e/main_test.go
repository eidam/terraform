...
func setTfeClient() {
	...
	if tfeHostname != "" && tfeToken != "" {
		...
		if err != nil {
			fmt.Println(fmt.Sprintf("Could not create new tfe client: %v", err))
			os.Exit(1)
		}
		tfeClient = client
	}
...
func setupBinary() func() {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("Could not create temp directory: %v", err))
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("Could not change directories: %v", err))
		os.Exit(1)
	}
	...
	err = cmd.Run()
	if err != nil {
		fmt.Println(fmt.Sprintf("Could not run exec command: %v", err))
		os.Exit(1)
	}
...
func writeCredRC(file string) {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("Could not create file: %v", err))
		os.Exit(1)
	}
	_, err = f.WriteString(creds)
	if err != nil {
		fmt.Println(fmt.Sprintf("Could not write credentials: %v", err))
		os.Exit(1)
	}
	f.Close()
}
