...
only, and is not applicable to Terraform Cloud-based configurations.

In Terraform Cloud mode, initialization always activates any new Cloud
...
The -reconfigure option is unsupported when entering or leaving Terraform
Cloud mode, because activating and deactivating Terraform Cloud mode involves
...
In Terraform Cloud mode, state storage is handled by Terraform Cloud and so
the state storage location is not configurable.
...
not applicable to Terraform Cloud-based configurations.
